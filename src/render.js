const renderChunck = ({timestamp, text}) => `
<div class="chunk flex">
    <time class="flex">00:00</time>
        <p>
...
window.seek = function(event) {
    const seekTo = event.currentTarget.dateset.seekTo
...
export function renderText({chunks}) {
    const formattedTranscription = chunks.map(renderChunck).join("")
